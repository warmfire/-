...
<script type="text/javascript">
	function isHidden() {
		...
		document.getElementById('againpwd-ww').style.display = 'none';
		...
		if (document.changepwd.againPwd.value.length == 0) {
		...
		if (document.changepwd.newPwd.value != document.changepwd.againPwd.value) {
			document.getElementById('againpwd-ww').style.display = 'block';
		}
...
<body>
	...
	<nav class="navbar navbar-default navbar-static-top">
		<div class="container-fluid">
			<div class="navbar-header">
				<a class="navbar-brand" href="/DFSystem/teacher.jsp?id=<%= request.getParameter("id")%>">打分系统 - 更改密码</a>
			</div>

			<div class="collapse navbar-collapse"
				id="bs-example-navbar-collapse-1">
				<ul class="nav navbar-nav navbar-right">
							<li><a href="/DFSystem/zunei.jsp?id=<%=request.getParameter("id")%>">返回</a></li>
					<li class="dropdown">
	...
	<div id="login-bg">
		<div id="login-cell">
		<form  action = "StudentServlet?method=13&id=<%= request.getParameter("id")%>" method = "post" name="changepwd">
			...
			<div id="againpwd-ww" class="alert alert-danger" role="alert"
				style="display: none;">两次密码不同，请重新输入</div>			
			...
			<div class="btn-group btn-block" role="group">
				<a href="/DFSystem/zunei.jsp?id=<%= request.getParameter("id")%>"><button type="button" class="btn btn-default  btn-block">返&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;回</button></a>
			</div>
		</form>
		</div>
	</div>
