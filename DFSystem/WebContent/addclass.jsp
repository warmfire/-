...
<script type="text/javascript">
	function isHidden() {
		...
		if (document.addclass.className.value.length == 0) {
			document.getElementById('mm-w').style.display = 'block';
		}
		if (document.addclass.className.value.length == 0) {
			document.getElementById('xm-w').style.display = 'block';
		}
		if (document.addclass.className.value.length != 0 && document.addclass.classId.value.length != 0)
...
<body>
	...
	<nav class="navbar navbar-default navbar-static-top">
		<div class="container-fluid">
			<div class="navbar-header">
				<a class="navbar-brand" href="/DFSystem/teacher.jsp?id=<%= request.getParameter("id")%>">打分系统 - 添加班级</a>
			</div>
	...
	<div id="login-bg">
		<div id="login-cell">
		<form  action = "StudentServlet?method=4&id=<%= request.getParameter("id")%>" method = "post" name="addclass">
						
			<div class="input-group btn-block">
				<input id="mm" type="text" class="form-control" placeholder="班级ID"
					aria-describedby="basic-addon1"  name="classId">
			</div>
			<div id="mm-w" class="alert alert-danger" role="alert"
				style="display: none;">请输入班级ID</div>
				
			<div class="input-group btn-block">
			<input id="xm" type="text" class="form-control" placeholder="班级"
					aria-describedby="basic-addon1"  name="className">
			</div>
			<div id="xm-w" class="alert alert-danger" role="alert"
				style="display: none;">请输入班级</div>				
